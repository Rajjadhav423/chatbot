...
if st.button("Predict Properties"):
    ...
    if predicted_density is not None and predicted_velocity is not None and predicted_intermolecular_free_length is not None:
        st.success(f"Predicted Seawater Density: {predicted_density:.6f} kg/m³")
        st.success(f"Predicted Ultrasonic Velocity: {predicted_velocity:.6f} m/s")
       st.success(f"Predicted Thermal Expansion Coefficient: {predicted_thermal_expansion_coefficient:.6f} K^-1")
            st.success(f"Predicted Adiabatic Compressibility: {predicted_Adiabatic_Compressibility:.6f} TPa^-1")
            st.success(f"Predicted Isothermal Compressibility: {predicted_Isothermal_Compressibility:.6f} TPa^-1")
